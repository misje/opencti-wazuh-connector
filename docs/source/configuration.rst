...
you need to specify **URLs**, **usernames**, **passwords** and **tokens**. It
is expected that most users of this connector is running the connector, with or
without the rest of OpenCTI, in :term:`docker`, using `docker-compose
...
- Environment variables specified as key–value entries as *key=value* on
  ...
  docker-compose.yml). The file may be named differently, depending on the
  setting :dcompose:`env_file <env_file>` in docker-compose.yml.
