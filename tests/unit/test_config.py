...
from wazuh.config import Config, EnrichmentConfig, OpenSearchConfig
...
def test_boolean_arg_true(trueish):
    assert Config(max_tlp="TLP:WHITE", agents_as_systems=trueish).agents_as_systems


def test_boolean_arg_flase(falseish):
    assert not Config(max_tlp="TLP:WHITE", agents_as_systems=falseish).agents_as_systems
